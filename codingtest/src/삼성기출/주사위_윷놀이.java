...
public class 주사위_윷놀이 {
    ...
    static class Node{ // 윷놀이 판을 연결 리스트 구조에 담는다.
        ...
        Node next;
        Node fastPath; // 윷놀이판에서 10,20,30은 두 가지 방향이 존재
    ...
    private static int gameStart(){
        ...
        int score = 0;
        for (int i = 1; i <= 10; i++){
            Node cur = horse[order[i]]; // 순열로 할당된 순서대로 말을 움직인다.
            cur.isEmpty = true; // 현재 있는 칸을 비워줍니다.

            for (int j = 1; j <= dice[i]; j++){ // 주사위에 나온 수치만큼 이동
                if (j == 1 && cur.fastPath != null) // 처음 이동을 시작하려는 위치가 파란색이라면.
                    cur = cur.fastPath; // 지름길로 이동 (파란색)
                else{
            ...
            if (!cur.isEmpty && !cur.isFinish){ // 이도을 마친 칸에 다른 말이 있다면, 해당 말은 고를 수 없다.
    ...
    public static void init(){
        ...
        end.next = end; // 자기자신을 가르키도록 설정 (도착 지점을 넘어서는 이동에 대헤 NPE 방지)
